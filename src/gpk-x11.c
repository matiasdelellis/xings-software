...
gpk_x11_set_xid (GpkX11 *x11, guint32 xid)
{
	...
	window = gdk_x11_window_foreign_new_for_display (x11->gdk_display, xid);
	if (window == NULL)
		return FALSE;

	/* save the x state */
	x11->display = GDK_DISPLAY_XDISPLAY (x11->gdk_display);
	x11->window = GDK_WINDOW_XID (window);
...
gpk_x11_get_user_time (GpkX11 *x11)
{
	...
	gdk_x11_display_error_trap_push (x11->gdk_display);
	atom = gdk_x11_get_xatom_by_name_for_display (x11->gdk_display, "_NET_WM_USER_TIME_WINDOW");
	rc = XGetWindowProperty (x11->display, x11->window, atom, 0, G_MAXLONG, False, XA_WINDOW,
				 &type, &format, &nitems, &bytes_after, (void*) &win);
	err = gdk_x11_display_error_trap_pop (x11->gdk_display);
	...
	gdk_x11_display_error_trap_push (x11->gdk_display);
	atom = gdk_x11_get_xatom_by_name_for_display (x11->gdk_display, "_NET_WM_USER_TIME");
	rc = XGetWindowProperty (x11->display, *win, atom, 0, G_MAXLONG, False, XA_CARDINAL,
...
gpk_x11_get_title (GpkX11 *x11)
{
	...
	/* get _NET_WM_NAME */
	gdk_x11_display_error_trap_push (x11->gdk_display);
	atom = gdk_x11_get_xatom_by_name_for_display (x11->gdk_display, "_NET_WM_NAME");
	atom_type = gdk_x11_get_xatom_by_name_for_display (x11->gdk_display, "UTF8_STRING");
	rc = XGetWindowProperty (x11->display, x11->window, atom, 0, G_MAXLONG, False, atom_type,
				 &type, &format, &nitems, &bytes_after, (void*) &data);
	gdk_x11_display_error_trap_pop_ignored (x11->gdk_display);
	...
	gdk_x11_display_error_trap_push (x11->gdk_display);
	rc = XGetWindowProperty (x11->display, x11->window, XA_WM_NAME, 0, G_MAXLONG, False, XA_STRING,
				 &type, &format, &nitems, &bytes_after, (void*) &data);
	gdk_x11_display_error_trap_pop_ignored (x11->gdk_display);
	if (rc == Success && nitems > 0) {
		title = g_strdup (data);
	...
	g_warning ("failed to get X11 name for window %i", (gint) x11->window);
out:
	if (data != NULL)
		XFree (data);
	return title;
...
gpk_x11_init (GpkX11 *x11)
{
	...
	x11->display = NULL;
	x11->window = None;
	x11->gdk_display = gdk_display_get_default ();
}
