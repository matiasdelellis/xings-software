...
gpk_task_setup_dialog_untrusted (GpkTask *task)
{
	...
	retval = gtk_builder_add_from_file (task->priv->builder_untrusted, PKGDATADIR "/gpk-error.ui", &error);
	if (retval == 0) {
...
gpk_task_setup_dialog_signature (GpkTask *task)
{
	...
	retval = gtk_builder_add_from_file (task->priv->builder_signature, PKGDATADIR "/gpk-signature.ui", &error);
	if (retval == 0) {
		g_warning ("failed to load ui: %s", error->message);
		g_error_free (error);
	}
...
gpk_task_setup_dialog_eula (GpkTask *task)
{
	...
	retval = gtk_builder_add_from_file (task->priv->builder_eula, PKGDATADIR "/gpk-eula.ui", &error);
	if (retval == 0) {
		g_warning ("failed to load ui: %s", error->message);
		g_error_free (error);
	}
